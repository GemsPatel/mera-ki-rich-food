...
class mdl_product extends CI_Model
{
	...
	function getData($srchKey = '')
	{
		if($this->cPrimaryId == "")
		{
			...
			$category_name = $this->input->get('cat_filter');
			$pin = $this->input->get('pin_filter');
			...
			if( isset($category_name) && $category_name!= "")
				$this->db->where('product_categories.category_id', $category_name );
			...
			if(isset($pin) && $pin != "")
			{
				$this->db->where('product.product_internal_note LIKE \'%'.trim($pin).'%\' ');
			}
			...
			$this->db->join('product_categories', 'product_categories.category_id=product.category_id', 'INNER');
...
// 								  ON (ss2_imsc.inventory_master_specifier_id=ss2_ims.inventory_master_specifier_id)	"; 
	...
	function saveItem( $is_external, &$logType )
	{
		...
		*/
		...
		{
			$dt_prod['product_price'] = $this->input->post('product_price');
		}
		...
		{
			$dt_prodval['product_value_quantity'] = $this->input->post('product_value_quantity');
		}
