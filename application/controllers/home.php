...
class home extends CI_Controller 
{
	 ...
	 *
	 */
	function videoDetailsTest()
	{
		$data['pageName'] = 'video_details';
		$this->load->view('site-layout',$data);
	}
...
*/	
	...
	function customerSignup()
	{
		...
		$data['email_address'] = 'cloudwebs17@gmail.com';//cloudwebstech
 ...
 */
	...
	function commonStringReplace()
	{
		...
		//stringReplaceCommon("product_categories_cctld","meta_description","category_cctld_id","Online Video Viewer.com","Online Video Viewer.com.au");
	}
	/**
	...
	{
		$toEmail = 'cloudwebsstore@gmail.com';
	...
	{
		$fromEmail = 'info@watch2earn.online';
	...
	{
		$data['pageName'] = 'products-details_bkp';
		$this->load->view('site-layout',$data);
	}
	 ...
	 *
	 */
	function recipeBlogDetails()
	{
	    $data['pageName'] = 'recipe-blog-details';
	    $this->load->view('site-layout',$data);
	}
...
// 		$comment .= "<br>Client side IP geolocation: <b>".$errorIPLocation."</b>";
	...
	function provideWatchVideo()
	{
		//get enabled customer id
		$customerArr = executeQuery( "SELECT customer_id, customer_firstname FROM customer WHERE customer_status = 0" );
		$dailyWatch = getField( "config_value" , "configuration", "config_key", "DAILY_WATCH" );
		
		if( !isEmptyArr( $customerArr ) )
		{
			foreach ( $customerArr as $customer )
			{
				echo $customer['customer_firstname']."<br>";
				$totalProvideVideo = exeQuery( "SELECT COUNT( customer_video_map_id ) as total FROM customer_video_map WHERE customer_id = ".$customer['customer_id']." AND cvm_status = 0 ");
				
				$leftProvideVideo = $dailyWatch - $totalProvideVideo['total'];
				echo "Total: ".$leftProvideVideo."<br>";
				if( !empty( $leftProvideVideo ) )
				{
					provideWatchVideo( $customer['customer_id'], $leftProvideVideo );
				}
			}
		}
		else
