...
class site extends CI_Controller {
	...
	function site()
	{
		...
		parent::__construct();
		$this->dbname = "cloudwebs_perry";
		$this->dbuser = "cloudwebs_perry";
	...
	function sku_transfer()
	{
		...
		$login = ftp_login($connection, 'cloudwebs', $this->config->item('encryption_key'));
...
//Read email from cpanel with imap in php
	function readEmailFromCpanel()
	{
		...
		$username = 'info@watch2earn.online';
		$password = 'cloudwebsfamily99';
 ...
 */		
	function folderServerBackup()
	{
		...
		$ftp_username = "cloudwebs";         // FTP account username
...
/**
 * @author Cloudwebs
 * Function will download diamonds from rapnet and insert/update diamond inventory at cloudwebs.
 */	
	function downloadInsUpdDiamonds()
	{
		...
		//header('Location: http://www.Online Video Viewer.com/site/processRapnetCsv?flp='.$filepath.'&starttime='.$starttime);
	}
	...
	function processRapnetCsv( $filepath='' )
	{
		...
		if( $start < $size )
		{
			...
			//initiate next batch process
			//header('Location: http://www.Online Video Viewer.com/site/processRapnetCsv');
		}
		else
		{
			//reset process index
	...
	function diamond_priceCcTld( $is_update, $diamond_price_id, &$data )
	{
		...
		else
		{
			...
			{
				$statusTemp = 1;
				if( $ar['manufacturer_id'] == 7 )	//primary Online Video Viewer.com
				{
					if( MANUFACTURER_ID != 7 )
					{
...
// 		}
	...
	function bugzillaLogin()
	{
		...
		$request = array('Bugzilla_login'=>'gautam.Cloudwebs@gmail.com', 'Bugzilla_password'=>'16other wise268', 
						'product'=>'ecommerce', 'component'=>'Online Video Viewer', 'summary'=>'Test Bug sec', 'version'=>'2.1.0', 
						'op_sys'=>'Windows', 'bug_severity'=>'Bug-fix_Enhancement', 'rep_platform'=>'PC',
						'assigned_to'=>'cloudwebs17@gmail.com', 'comment'=>'Test First rpc call', 'keywords'=>'HE_P1_',
						'priority'=>'High' );
...
// 		}
	...
	function getCurlImage()
	{
		$content = file_get_contents( 'https://www.deeparteffects.com/images/generated/09c7596d-0fb9-491c-9ee9-5873fb7f45d6.jpg'  );
