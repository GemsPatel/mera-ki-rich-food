...
*/	
	...
	/*
	 ...
	 +-------------------------------------------------------+
	 */
	function checkMobileDuplication($str)
	{
		//Guest User
		
		if( checkIfRowExist( "SELECT 1 FROM customer WHERE customer_phoneno='".$str."' " ) )
		{
			//change : 01-05-2015 Check mail for password remove message for guest user.
			$this->form_validation->set_message('checkMobileDuplication','This Mobile Number already registered with us.');
			return false;
		}
		else
		{
				...
				else
				{
					...
					if($response && ($response['customer_emailid'] == $email))
					{
						...
						else
						{
							...
							$subject = 'Reset Your Password at '.baseDomain();
