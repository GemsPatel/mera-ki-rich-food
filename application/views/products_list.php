<section class="shop">
	<div class="container">
		<div class="row">
			<?php //$this->load->view('elements/leftbar-filter'); ?>
			<form name="comparef" id="comparef" method="get" action="#">
				<div class="col-lg-12 col-sm-12 col-sm-12 padbot20">
					<div class="sorting_options clearfix">
						<?php 
						$records = (int)$total_records." items found.";
						$per_page = PER_PAGE_FRONT;
						if($total_records>PER_PAGE_FRONT):
						?>
							<script type="text/javascript">
								var is_records = true;
							</script>
						<?php
						else:
						?>
							<script type="text/javascript">
								var is_records = false;
							</script>
						<?php	
							endif;
						?>
						<div class="count_tovar_items">
							<h1>
								<?php
// 								$segment = $this->uri->segment();
								$title = end($this->uri->segment_array());
								echo pgTitle( str_ireplace( ".html", "", $title ) ); 
								?>
							</h1>
							<span><?php echo $records;?></span>
						</div>
					</div>

					<div class="row shop_block" id="right-catagory">
						<?php 
						if(isset($listArr) && sizeof($listArr)>0)
						{
				    		foreach($listArr as $key=>$val)
				    		{
					    		$catidArr = explode("|",$val['category_id']);
					    		if(is_array($catidArr) && sizeof($catidArr)>0)
					    		{
					    			$val['category_id'] = $catidArr[0];
					    		}
					    		
					    		$prodUrl = getProductUrl($val['product_id'],$val['product_price_id'],$val['product_alias'],$val['category_id']);
					    		
					    		$product_images = front_end_hlp_getProductImages($val['product_generated_code'], $val['product_price_id'], $val["product_sku"], $val['product_generated_code_info']);?>

								<div class="tovar_wrapper col-lg-3 col-md-3 col-sm-4 col-xs-6 col-ss-12 padbot40" itemscope itemtype="http://schema.org/Product">
									<div class="resent-grid-img recommended-grid-img">
										<div class="hovereffect">
									        <img class="img-responsive imageHeightWidth" src="<?php echo load_image( $product_images[$val['product_angle_in']] )?>" alt="<?php echo $val["product_name"];?>">
									        <div class="overlay">
									            <h2><?php echo char_limit( $val["product_name"], 25 );?></h2>
									            <a class="info" href="<?php echo $prodUrl;?>">Whatch Now</a>
									        </div>
									    </div>
										<div class="time hide">
											<p><?php echo $val['product_internal_note']?></p>
										</div>
										<div class="clck hide">
											<span class="glyphicon glyphicon-time" aria-hidden="true"></span>
										</div>
									</div>
									<div class="resent-grid-info recommended-grid-info">
										<ul>
											<li>
												<p class="author author-info">
													<a href="<?php echo $prodUrl;?>" class="author"><?php echo char_limit( $val["product_name"], 35 );?></a>
												</p>
											</li>
											<!-- <li><p class="author author-info"><a href="#" class="author">John Maniya</a></p></li> -->
											<!-- <li class="right-list"><p class="views views-info">2,114,200 views</p></li> -->
										</ul>
									</div>
								</div>
							<?php
				    		}
						}
						else
						{
							echo '<div class="col-md-9"><h3><i class="fa fa-thumbs-o-down"></i> &nbsp;Sorry, Your search criteria did not match.</h3></div>';
						} ?>
					</div><!-- //ROW -->
					<?php if( false && $records > $per_page):?>
						<hr>
						<div class="clearfix">
							<ul class="pagination">
								<?php echo productListPagination($start, $per_page, $total_records, $this->input->server('REQUEST_URI'));?>
								<div class="active" id="page_no"></div>
							</ul>                            
						</div>
						<hr>
					<?php endif;?>
				</div>
			</form>
		</div><!-- //ROW -->
	</div><!-- //CONTAINER -->
</section>
<?php 
if( true || isIntranetIp() )
{
	?>    
	<script type="text/javascript">
	var cz = '<?php //echo $this->cz?>';
	var is_called = false;
	is_listing_page = true;  
	filter_page = 'filter';
	
	/**
	 * @author Cloudwebs
	 * @abstract used in page scroll pagination
	 */
	$(window).scroll(function(e)
	{
		scrPagination(e);
	});
	</script>
	
	<?php
}
?>