...
function getLiveGoldPrice()
{
	try {
		...
		$vec = array("UserName"=>"cloudwebs17@gmail.com","Password"=>"cloudwebs");
 ...
 */
function sendSMS($mo_no,$msg, $id='dndcloudwebs_pri')
{
	return;
...
*/	
	...
	function createMeta( $search_tag )
	{
		...
		$resArr['custom_page_title'] = "Watch ".$search_tag;
		$resArr['meta_description'] = "Video listing of ".$search_tag; 			
		$resArr['meta_keyword'] = $search_tag.", Watch online ".$search_tag;
		return $resArr; 			
	}
	...
	*/
	...
	/**
	 * this function will return domain prefixed url for cloudwebs assets 
	 */
	...
	function errorLog( $key, $msg, $is_mail= true, $is_db=true, $is_useCIMail=false, $additional_DEV_EMAIL="" )
	{
		...
		if( $is_mail )
		{
			if( $is_useCIMail )
			{
				sendMail( 'cloudwebs17@gmail.com'.$additional_DEV_EMAIL, "Online Video Viewer: Instant error notification from error log KEY: ".$key, $msg);
			}
			else
			{
				//log to email
				// ini_set("log_errors", 1); //ini_set("error_log", "/tmp/php-error.log");
				$headers = "From: cloudwebs17@watch2earn.online\r\n";
				$headers .= "MIME-Version: 1.0\r\n";
				$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
				error_log ( 'Subject: '.$key. ' <br><br>Message: ' .$msg, 1, 'cloudwebs17@gmail.com', $headers );
...
{
	if( $_SERVER['REMOTE_ADDR'] == '2405:204:8406:1b19:2cd3:9688:5910:aae' || issetCheck( $_GET, "isIntranetIp" ) == 1 ) 
	{
		return true;
	}
	else
...
function getRequestUri()
{
	...
	{
		return str_replace( "/Online Video Viewer/", "/", $CI->input->server('REQUEST_URI'));
	}
}
...
function bugzilla_RPC_Call( $summary, $op_sys, $comment, $rep_platform )
{
	...
	$request = array('Bugzilla_login'=>'gautam.Cloudwebs@gmail.com', 'Bugzilla_password'=>'16other wise268',
			'product'=>'ecommerce', 'component'=>'Online Video Viewer', 'summary'=>$summary, 'version'=>'2.1.2',
			'op_sys'=>$op_sys, 'bug_severity'=>'Bug-fix_Enhancement', 'priority'=>'High',
			'assigned_to'=>'cloudwebs17@gmail.com', 'comment'=>$comment, 'keywords'=>'CRASH_LOG',
...
/**
 * Detect Bowser run is mobile device or desktop
 * @return boolean
 */
function isRunMobileDevice(){
	$aMobileUA = array(
			'/iphone/i' => 'iPhone',
			'/ipod/i' => 'iPod',
			'/ipad/i' => 'iPad',
