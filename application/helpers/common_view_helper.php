	...
	else 
	{
		...
		if( $is_search )
		{
			if( $res['search_code'] == "" || $res['search_code'] == " Products " )
			{
				if( !$is_no_result )
				{
					$res['search_code'] = " Videos...";
				}
				else
				{
					$res['search_code'] = " No Videos... matching your criteria.";
				}
			}
			 ...
			 */
			$search_tag = str_ireplace( "Buy", "Watch", $search_tag );
			$search_tag = str_ireplace( "Product", "Video", $search_tag );
...
function cmn_vw_signup()
{
	...
	$CI->form_validation->set_rules('customer_phoneno','Phone','trim|required|numeric|callback_checkMobileDuplication');
	...
	if(is_restClient())
	{
		...
		else
		{
			...
			if($response && ($response['customer_emailid'] == $email))
			{
				...
				$subject = 'Reset Your Password at Online Video Viewer';
...
function cmn_vw_socialmediashare_get( &$__this )
{
	...
	if( is_restClient() )
	{
		...
		if( !isEmptyArr( $res ) )
		{
			...
			$cmn_vw['paragraph'].= " <br/> http://www.watch2earn.online/home/invitedFriends?ref=".$CI->session->userdata("customer_emailid");
			$cmn_vw['paragraph'].= " <br/><br/> Regards, <br/> ".$c_name;
			
			$cmn_vw['paragraph_str1'] = str_replace( "&nbsp;", '', strip_tags($res['article_description']) );
			$cmn_vw['paragraph_str2']= "http://www.watch2earn.online/home/invitedFriends?ref=".$CI->session->userdata("customer_emailid");
...
function provideWatchVideo( $customer_id, $leftProvideVideo )
{
	$CI =& get_instance();
	$productArr = executeQuery( "SELECT product_id, product_name FROM product WHERE product_status = 0 ORDER BY RAND() LIMIT 0, ".$leftProvideVideo );
	...
	foreach ( $productArr as $product )
	{
		...
		$CI->db->insert( 'customer_video_map', $map );
	}
}
...
{
	if( $day == 1 )
		return 'àª¸à«‹àª®àªµàª¾àª°';
	else if($day == 2)
		return 'àª®àª‚àª—àª³àªµàª¾àª°';
	else if($day == 3)
		return 'àª¬à«�àª§àªµàª¾àª°';
	else if($day == 4)
		return 'àª—à«�àª°à«�àªµàª¾àª°';
	else if($day == 5)
		return 'àª¶à«�àª•à«�àª°àªµàª¾àª°';
	else if($day == 6)
		return 'àª¶àª¨àª¿àªµàª¾àª°';
	else
		return 'àª°àªµàª¿àªµàª¾àª°';
}