 ...
 * connection to the database here will be from cloudwebs.com because only there price are updatted each hour
 */	
	function avgSolitaireDiamondPrice( $diamond_price_id, $diamond_shape_id, $weight, $diamond_color_key="'I'", $diamond_purity_key="'SI1'", $callNo=0, $endWeight=0 )
	{
		...
		//$conn = mysql_connect('www.cloudwebs.com', 'localcon', '1,)8KLc]ll~$');
		//var_dump($conn);
 ...
 */	
	...
	/**
	 * function will return all department/store/ccTLD that cloudwebs working with
	 */
...
*/	
	function sendChatStartMsg( $admin_id, $admin_user_firstname, $mono, $user_id, $chat_id)
	{
		...
		sendSMS( parseMobileNo( $mono ), "cloudwebs Chat: ". joinChatRoomLink( $admin_id, $chat_id), 'cloudwebs_pri');
		
		//Temp
		sendMail( "cloudwebs17@gmail.com", "cloudwebs chat ", "cloudwebs Chat: ". joinChatRoomLink( $admin_id, $chat_id));
	}
