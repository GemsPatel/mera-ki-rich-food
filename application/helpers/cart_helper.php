				...
				if( !isset($resArr['cart_prod'][ $k ]['not_available']) )
				{
					if( empty( $ar['type'] ) || $ar['type'] == 'prod' )
					{
						...
						else
						{
							...
							//transaction re commit
							setFlashMessage( 'error', "Something wrong happen.Contact support@watch2earn.online, Order ID: ".$resOrdArr['order_id']."" ); 
							redirect( 'admin/'.$CI->controller ); 
						}
					}
					else if( $ar['type'] ==  'cz' )
...
function cart_hlp_orderConfirmMail( $is_front_end, $resArr ) 
{
	...
	if( $is_front_end )
	{
		...
		//[temp]: DEBUG: send email to cloudwebs17@gmail.com
		$data_email['es_to_emails'] = "cloudwebs17@gmail.com";
		sendMail( $data_email['es_to_emails'], $data_email['es_subject'], $data_email['es_message']);
		...
		{	
			...
			if( isset($resArr['payment_method_id']) && $resArr['payment_method_id']==4 )
				$msg = 'Thank you for shopping at watch2earn.online. Your COD order '.$resArr['order_id'].' will be confirmed by our customer executive very soon.';
			else if($resArr['order_status']=='S')
				$msg = 'Thank you for shopping at watch2earn.online. Your order '.$resArr['order_id'].' is confirmed and being  processed. You will be notified once your order gets shipped.';
	...
	function orderEmail( $order_id, $order_status_key, $order_status_msg , $order_tracking_number, $currency_id=0, $is_return=false )
	{
		...
		if( !empty($order_id) )
		{
			...
			if( !$is_return )
			{
				...
				//DEBUG: send email to cloudwebs17@gmail.com
				$data_email['es_to_emails'] = "cloudwebs17@gmail.com";
				sendMail( $data_email['es_to_emails'], $data_email['es_subject'], $data_email['es_message']);	
				$CI->db->insert("email_send_history",$data_email);
